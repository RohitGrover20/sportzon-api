  ...
  getDashboard: async (req, res) => {
    try {
      ...
      const [
        ...
        Events.aggregate([
          { $match: { eventDate: { $exists: true, $ne: null } } },
          { $group: { _id: { $month: { $toDate: "$eventDate" } }, count: { $sum: 1 } } },
          { $project: { month: { $let: { vars: { monthArray: MONTHS }, in: { $arrayElemAt: ["$$monthArray", { $subtract: ["$_id", 1] }] } } }, count: 1, _id: 0 } },
        ]),
        Users.aggregate([
          { $group: { _id: { $month: { $toDate: "$createdAt" } }, count: { $sum: 1 } } },
          { $project: { month: { $let: { vars: { monthArray: MONTHS }, in: { $arrayElemAt: ["$$monthArray", { $subtract: ["$_id", 1] }] } } }, count: 1, _id: 0 } },
        ]),
      ]);

      // Initialize arrays with counts set to 0 for all months
      const monthlyEventCountsArray = Array.from({ length: 12 }, (_, monthIndex) => ({ month: MONTHS[monthIndex], count: 0 }));
      const monthlyUserCountsArray = Array.from({ length: 12 }, (_, monthIndex) => ({ month: MONTHS[monthIndex], count: 0 }));
