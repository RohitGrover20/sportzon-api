...
const EventSchema = mongoose.Schema({
    ...
    prize: { type: Number, required: () => { this.isPrizeIncluded == true ? true : false } },
    entryFees: { type: Number, required: true },
    memberType: { type: String, required: true, eval: ["team", "individual"] },
    memberLimit: { type: Number, required: () => { this.memberType == "team" ? true : false } },
    totalSlots: { type: Number, required: true },
    ticketSystem: { type: Boolean, required: true },
    emptySlots: { type: Number, required: true, default: () => this.totalSlots },
